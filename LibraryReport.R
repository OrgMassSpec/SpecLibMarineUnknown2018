...
LibraryReport <- function(spectra = spec, 
  metadata = meta,
  pdfFile = "SpecLibMarineUnknown2018.pdf", 
  pdfTitle = "SpecLibMarineUnknown2018 Library") {
  ...
  pushViewport(viewport(layout = grid.layout(nrow = 4, ncol = 1, heights = unit(c(0.1, 0.2, 0.5, 0.2), "npc"))))
    pushViewport(viewport(layout.pos.row = 1))
      grid.text("SpecLibMarineUnknown2018 Mass Spectral Library", y = 0.5, gp = gpar(cex = 1.25))
    ...
    pushViewport(viewport(layout.pos.row = 4))
      ...
      grid.text(paste("SpecLibMarineUnknown2018"), y = 0.65)
  ...
  DrawPage <- function(currentCompound) {
    ...
    pushViewport(viewport(layout = spec.layout))
      ...
      pushViewport(viewport(layout.pos.row = 2))
        ...
        grid.text(paste("In Bottlenose Dolphin: ", currentMetadata$Bottlenose, sep = ""), x = 0, y = 0.5, hjust = 0)                             
        grid.text("Instrument: GCxGC-TOF, EI Mode", x = 0.33, y = 0.75, hjust = 0)
        grid.text(paste("1D RT, 2D RT (s): ", currentMetadata$RetentionTimes, sep = ""), x = 0.33, y = 0.5, hjust = 0)   
